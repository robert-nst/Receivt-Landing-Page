...
import WebsiteSender from "./sender"
  ...
  const getStepContent = () => {
    switch (step) {
      case 1:
        return (
          <WebsiteSender
            InputString={brandName}
            setInputString={setBrandName}
            onNext={handleNext}
          />
        )
      case 2:
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-8">
              {(["card", "points", "tiers"] as LoyaltyType[]).map((type) => (
                <div key={type} className="flex flex-col items-center">
                  ...
                  />
                  <Button
                    className={`bg-[#26a269] hover:bg-[#26a269]/90 text-white w-full mt-4 px-8 py-6 text-lg ${loyaltyType === type ? "ring-2 ring-offset-2 ring-[#083118]" : ""
                      }`}
                    onClick={() => {
                      setLoyaltyType(type)
                      handleNext()
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-8">
              {(["coupons", "discounts", "points"] as PromotionsType[]).map((type) => (
                <div key={type} className="flex flex-col items-center">
                  ...
                  />
                  <Button
                    className={`bg-[#26a269] hover:bg-[#26a269]/90 text-white w-full mt-4 px-8 py-6 text-lg ${promotionsType === type ? "ring-2 ring-offset-2 ring-[#083118]" : ""
                      }`}
                    onClick={() => {
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-8">
              {(["list", "recent"] as ReceiptsType[]).map((type) => (
                <div key={type} className="flex flex-col items-center">
                  ...
                  />
                  <Button
                    className={`bg-[#26a269] hover:bg-[#26a269]/90 text-white w-full mt-4 px-8 py-6 text-lg ${receiptsType === type ? "ring-2 ring-offset-2 ring-[#083118]" : ""
                      }`}
                    onClick={() => {
      ...
      <div className="flex flex-col items-center bg-transparent">
        <div className="mb-4 flex justify-center gap-4">
          {previewTabs.map(tab => (
            <button
              key={tab.key}
              className={`px-6 py-2 rounded-full text-lg font-semibold transition-colors ${activeTab === tab.key
